...
def loss_size_test(
    sizes: list, loss_test_list: list, number_of_learning: int = 100
) -> None:
    """
        ...
        for loss in loss_test_list:
            for b in [True, False]:
                ...
                W = nmap(
                    ...
                    test_n(
                        ch,
                        n=number_of_learning,
                        loss_f=loss,
                        sort=b,
                        size=size,
                        pre_print="Size {} :: {}".format(size, lab),
                        quiet=False,
                    ),
                )
...
def loss_learn_test(
    number_of_learning: list, loss_test_list: list, size: int = 100
) -> None:
    """
    test some loss functions
    ...
    for n in number_of_learning:
        for loss in loss_test_list:
            for b in [True, False]:
                tmp = template.format(
                    n=n, loss=loss.__name__, b=str(b).lower(), size=size
                )

                W = test_n(
                    ch, n=n, loss_f=loss, sort=b, size=size, pre_print=tmp, quiet=False
                )
